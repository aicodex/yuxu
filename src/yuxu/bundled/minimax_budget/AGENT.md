...
| `agent_usage` | `{agent?}` | `{ok, usage: [{agent, model, requests, total_tokens, avg_tokens_per_req, calls_1h, tokens_1h, avg_tokens_per_req_1h}]}` |
| `estimate` | `{agent, n_requests}` 或 `{agent, n_tokens}` | `{ok, projected_*, avg_tokens_per_req, cost_per_call, cost_source, calls_1h, tokens_1h}` |
| `cost_per_call` | `{agent}` | `{ok, value, source, calls_1h, tokens_1h, ...}` — hot-path API（rate_limit / llm_driver） |
| `can_serve` | `{agent}` | `{ok, allowed, reason, remaining, own_reserved, own_used_in_interval, reserved_for_others}` — reservation gate |
| `refresh` | `{account_id?}` | 立即轮询一次（绕过缓存） |
| `reset_local` | `{agent?}` | 清本地归因（调试用） |

## Reservation（v0.2，预留额度）

给某些 agent 在 5h interval 窗口里预留一块**绝对 floor**，其他 agent 不能
把剩余消耗到侵蚀这块的地步。典型场景：定时任务必须按时发。

配置（env）：
```
MINIMAX_RESERVATIONS='{"scheduled_newsfeed": 50, "critical_trader": 30}'
```
单位是"**5h interval 里的请求数**"。无效 JSON / 非正整数自动跳过并 warn，
不炸启动。

`can_serve(agent)` 判定链：
1. 无 reservation 配置 → `allowed=True reason=no_reservations`
2. 启动初期还没拉到 MiniMax 数据 → `allowed=True reason=no_remaining_data`
3. agent 有自己的 reservation 且当前 interval 用量未到配额 →
   `allowed=True reason=own_floor`（走自己的 floor）
4. 否则看自由池：`remaining > Σ(其他 agent 未用预留)` →
   `allowed=True reason=free_pool`，否则 `allowed=False reason=reserved_for_others`

**interval rollover**：目前是本地 5h `time.monotonic()` 窗口滚动，rollover
时清空 `_interval_requests`。Phase 5 打算对齐 MiniMax 权威
`interval.start_time`——现在会有轻微 drift。

**消费者**：
- `scheduler`：opt-in `SCHEDULER_RESERVATION_CHECK=1`，fire 前 `can_serve(target)`，
  拒绝发 `scheduler.skipped reason=reservation_locked`（带 diagnostic）。
- `rate_limit_service`：暂未接入（避免双检开销；要接直接在 `acquire()` 里加）
