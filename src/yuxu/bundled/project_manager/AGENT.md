...
list_projects / list_agents）已下沉为 skill，见 `yuxu/skills_bundled/`。
...
```bash
yuxu init <dir>                    → skills_bundled/create_project
yuxu new agent <name>              → skills_bundled/create_agent
yuxu list projects                 → skills_bundled/list_projects
yuxu list agents                   → skills_bundled/list_agents
...
- LLM 想"建项目/建 agent"应通过 skill_picker → 各 skill；不再走本 agent
